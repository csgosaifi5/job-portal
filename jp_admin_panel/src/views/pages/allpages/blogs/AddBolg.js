...
const AddBlog = () => {
  ...
  const getSectionDetails = async () => {
    let response = await blogServ.getBlog(id)
    if (response) {
      setInitialValues({...response,tags:JSON.parse(response.tags)})
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      if (id) {
        ...
        response = await blogServ.addBlog(formData)
      }
      if (response.error) {
    ...
    <CCard style={{ margin: '0 auto 1.5rem' }}>
      ...
      >
        ...
        <div>
          ...
          <CButton color="primary" variant="outline" onClick={() => navigate('/blogs/list-all')}>
            Back
          </CButton>
        </div>
      </CCardHeader>
        ...
        <CCardBody>
          <CForm onSubmit={formik.handleSubmit}>
            ...
            <CCard className={'d-flex flex-column gap-3 p-3'}>
              ...
              <CRow>
                <CCol sm="12" md="6">
                  <CFormLabel htmlFor="tags">Related Tags*</CFormLabel>
                  <CreatableSelect
                    type="text"
                    id="tags"
                    placeholder="Enter Tag name for similarity"
                    ...
                    onBlur={formik.handleBlur}
                    className={
                      'mb-2' + (formik.touched.tags && formik.errors.tags ? ' border-danger' : '')
                    }
                    isMulti
