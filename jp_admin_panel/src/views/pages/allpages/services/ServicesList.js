...
const ServicesList = () => {
  ...
  const getServicesList = async () => {
    let result = await AtsServ.listAll(search)
    if (result.rows) {
      setServicesList((prev) => result.rows)
  ...
  const deleteRec = async () => {
    try {
      if (showDeletePopup.afId) {
        let response = await AtsServ.deleteBlog(showDeletePopup.afId)
        if (response.message) {
  ...
  const activeToggleHandler = async (value) => {
    try {
      ...
      let response = await AtsServ.edit(obj)
      if (response.message) {
        getServicesList()
        toast.success(response.message)
      }
        ...
        <CCardBody style={{ overflowX: 'auto' }}>
          <CTable>
            ...
            <CTableBody>
              {servicesList.map((el) => {
                return (
                  <CTableRow key={'page' + el.service_id}>
                    ...
                    <CTableDataCell>
                      <div className="d-flex gap-3 align-items-center mt-1">
                        <CIcon
                          icon={cilPencil}
                          onClick={() => navigate(`/services/edit/${el.service_id}`)}
                          className="text-success cursorPointer"
                        />
                        <CIcon
                          icon={cilTrash}
                          onClick={() => {
                            setShowDeletePopup({ show: true, afId: el.service_id })
                          }}
