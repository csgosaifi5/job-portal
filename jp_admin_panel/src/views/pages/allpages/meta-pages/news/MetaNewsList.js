...
const MetaNewsList = () => {
  ...
  const getPagesList = async () => {
    let result = await pagesService.listAll({ screen_name: 'News And Updates' })
    if (result.data) {
  ...
  return (
    <CCard style={{ margin: '0 auto 1.5rem', overflowX: 'hidden' }}>
      <CCardHeader className="d-flex flex-row justify-content-between">
        <p className="fw-bolder fs-4 m-0">Edit News Sections</p>
        <CTableDataCell>
