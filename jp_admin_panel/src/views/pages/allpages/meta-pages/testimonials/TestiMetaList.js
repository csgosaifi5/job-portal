...
const TestiMetaList = () => {
  ...
  const getPagesList = async () => {
    let result = await pagesService.listAll({ screen_name: 'Testimonials' })
    if (result.data) {
  ...
  return (
    <CCard style={{ margin: '0 auto 1.5rem', overflowX: 'hidden' }}>
      <CCardHeader className="d-flex flex-row justify-content-between">
        <p className="fw-bolder fs-4 m-0">Edit Testimonial Page</p>
        <CTableDataCell>
                ...
                return (
                  <CTableRow key={'page' + el._id}>
                    ...
                    <CTableDataCell style={{textAlign:"center"}}>
                        ...
                        <Link
                          to={`/edit/testimonials/${el.section_name
                            .replaceAll(' ', '-')
                            .toLowerCase()}/${el._id}`}
                        >
