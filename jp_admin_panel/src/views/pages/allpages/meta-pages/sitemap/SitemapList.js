...
const SitemapList = () => {
  ...
  const getPagesList = async () => {
    let result = await pagesService.listAll({ screen_name: 'Sitemap' })
    if (result.data) {
  ...
  return (
    <CCard style={{ margin: '0 auto 1.5rem', overflowX: 'hidden' }}>
      <CCardHeader className="d-flex flex-row justify-content-between">
        <p className="fw-bolder fs-4 m-0">Edit Sitemap Sections</p>
      </CCardHeader>
      {!contentLoaded ? (
          ...
          <CTable>
            <CTableHead>
              <CTableRow>
                ...
                <CTableHeaderCell style={{ textAlign: 'center' }} scope="col">
                  Actions
                </CTableHeaderCell>
              </CTableRow>
            </CTableHead>
                ...
                return (
                  <CTableRow key={'page' + el._id}>
                    ...
                    <CTableDataCell style={{ textAlign: 'center' }}>
                      <Link
                        to={`/edit/sitemap/${el.section_name.replaceAll(' ', '-').toLowerCase()}/${
                          el._id
                        }`}
                      >
